...
if 'ZeroBaselineStatus' not in st.session_state:
    st.session_state.ZeroBaselineStatus = "**Zero Baseline Not Set**"
...
def FindMVC(daq_input, durationCPM=21, window_size=50):
    time_vals, torque_vals = helperMVC(daq_input, durationCPM, window_size)

    readings = np.column_stack((time_vals, torque_vals))
    st.session_state.MVC = readings

def start_task(daq_input, durationCPM=10, num_cycles=1, window_size=50):

    trialLength = durationCPM * num_cycles

    MVC = st.session_state.MVC

    repeated_MVC = []
    for i in range(num_cycles):
        time_offset = i * MVC[-1, 0]
        segment = MVC.copy()
    ...
    y_top_range = math.ceil(st.session_state.MVC[:,1].max() + (.1 * st.session_state.MVC[:,1].max()))
    y_bottom_range = math.floor(st.session_state.MVC[:,1].min() - (.1 * abs(st.session_state.MVC[:,1].min())))
...
def create_components():
    ...
    with st.expander("Settings", expanded=True):
        ...
        with col1:
            ...
            if st.button("Set MVC"):
                FindMVC(daq_input,durationCPM)
        with col2:
    ...
    with st.expander("Stopping", expanded=True):
        if not isinstance(st.session_state.MVC, list):
            print(len(st.session_state.MVC))
            fig, ax = plt.subplots()
            ax.plot(st.session_state.MVC[:,0], st.session_state.MVC[:,1], label='unsmooth value', linewidth=2)
            ax.set_xlabel('Time')
